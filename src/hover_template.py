...
def get_hover_univ(x_freq, y_freq, universities):
    ...
    hovertext_doctorat = [f'<b>Doctorat</b><br>Université: {uni}<br>No. de thèses: {y_freq[uni] if uni in y_freq else 0}' for uni in universities]
...
def get_hover_discipline():
    ...
    hovertemplate_doctorat = '<b>Doctorat</b><br>Discipline: %{theta}<br>No. de thèses: %{r}<extra></extra>'
...
def get_hover_sunburst_chart_langue():
    hover_temp = "<b>%{id}</b><br><b>Count:</b> %{value}<extra></extra>"
    return hover_temp
